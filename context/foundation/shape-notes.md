...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "import of invalid file"
      decision: "rejected with a clear error"
...
3. User also adds C → A. The application shows a cyclic-dependency warning for A → B → C → A.
4. User removes C → A. The warning disappears.

   *(Revised in Socrates round: steps 3–4 become — the user tries to add C → A; the application rejects it and explains it would create the cycle A → B → C → A. See FR-003, US-01.)*
5. User sets the project start date on START and Durations for A, B, C. Critical-path and total-work project finish dates are now calculated. They are the same, as no parallelism is possible.
6. User adds Task D that depends on A and on which C depends: A → D → C. Project finish dates are no longer calculated; the application shows that Durations are missing.
7. User adds a Duration to Task D. Critical-path and total-work finish dates are calculated again, and now differ due to possible parallelism.
8. User exports the project to a file. The user is asked for a project name if they did not set it earlier; this is also the name of the exported file.
9. User opens the application in another browser; the project is empty.
10. User imports the previously exported file. The application shows the same data as in the previous browser.

Also in MVP (not exercised in the flow above): critical path(s) highlighted on the diagram; tasks/dependencies failing validation highlighted on the diagram; task statuses (To Do / In Progress / Done) with completion date for Done tasks; forecast that takes the current date and actual completion dates of Done tasks into account.

## Success Criteria

### Primary
- The MVP flow above works end to end: the user builds a task graph, sees a cycle flagged and cleared, gets critical-path and total-work finish dates once all Durations and the start date are set, sees them withheld while Durations are missing, and round-trips the project through an exported file into a different browser.
...
- When the project fails validation (e.g. cycle, missing Durations), no finish date is shown rather than a misleading one.
...
- FR-002: User can rename and delete a Task. Priority: must-have
  > Socrates: Counter-argument considered: "renames rewrite references; deleting a Task orphans its dependents." Resolution: references use ids, so renames don't touch dependencies. Deleting a Task may cause a validation warning (e.g. Tasks no longer on a path between START and FINISH) or simply a forecast recalculation.
- FR-003: User can add and remove predecessors of a Task; a dependency that would create a cycle, give START a predecessor, or make a Task depend on FINISH is rejected at input with an explanation (for a cycle, naming it). Priority: must-have
...
- FR-004: User can set a Duration on a Task as a positive whole number of days. Priority: must-have
...
- FR-005: User can set a Task's Status (To Do, In Progress, Done) and record its completion date when Done; marking Done is rejected when any predecessor is not Done, or when the completion date is after today, before the START date, or before a predecessor's completion date. Priority: must-have
...
### Validation
- FR-009: User can see validation problems — e.g. missing Durations — with the failing Tasks highlighted on the diagram; no finish date is shown while any problem exists. Priority: must-have
  > Socrates: Counter-argument considered: "every new Task without a successor would be an error until linked." Resolution: Tasks without successors automatically feed FINISH (mirroring Tasks without predecessors depending on START). Note: with cycles blocked at input (FR-003), auto-links to START/FINISH, and invalid imports rejected (FR-014), cycle and START→FINISH path problems cannot arise in a project inside the app; they remain checks applied to imported files.
...
- FR-011: User can see critical-path time and total-work time of the remaining (not-Done) work for a project that passes validation. Priority: must-have
...
### Persistence
- FR-013: User can export the project to a plain-text file named after the project, and is asked for a project name if none is set. Priority: must-have
...
- FR-014: User can import a previously exported file and get exactly the same project back; an invalid file (e.g. cycle, unknown ids, broken START/FINISH rules) is rejected with a clear error. Priority: must-have
  > Socrates: Counter-argument considered: "hand-edited files may be invalid." Resolution: invalid files are rejected with a clear error.
...
- **Then** the dependency is not created, and the application explains that it would create the cycle A → B → C → A
...
### US-02: Forecast appears only for a complete, valid project

- **Given** a valid project START → A → B → C → FINISH with a START date and Durations on A, B, C
...
#### Acceptance Criteria
- After adding Task D with A → D → C and no Duration, the finish dates disappear and the application indicates that D is missing a Duration.
...
- Marking B as Done while A is not Done is rejected; a completion date after today, before the START date, or before A's completion date is rejected.
